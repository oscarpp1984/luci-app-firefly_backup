...
define Package/luci-app-firefly
 	SECTION:=luci
	CATEGORY:=LuCI
...
define Package/luci-app-firefly/conffiles
/etc/ssr_ip
/etc/dnsmasq.firefly/gfw_list.conf
endef

define Package/luci-app-firefly/install
	$(INSTALL_DIR) $(1)/usr/lib/lua/luci
	cp -pR ./luasrc/* $(1)/usr/lib/lua/luci
	$(INSTALL_DIR) $(1)/
	cp -pR ./root/* $(1)/
	$(INSTALL_DIR) $(1)/usr/lib/lua/luci/i18n
	po2lmo ./po/zh-cn/firefly.po $(1)/usr/lib/lua/luci/i18n/firefly.zh-cn.lmo
endef

define Package/luci-app-firefly/postinst
#!/bin/sh
if [ -z "$${IPKG_INSTROOT}" ]; then
	( . /etc/uci-defaults/luci-firefly ) && rm -f /etc/uci-defaults/luci-firefly
	rm -f /tmp/luci-indexcache
	chmod 755 /etc/init.d/firefly >/dev/null 2>&1
	/etc/init.d/firefly enable >/dev/null 2>&1
fi
exit 0
endef

define Package/luci-app-firefly/prerm
#!/bin/sh
if [ -z "$${IPKG_INSTROOT}" ]; then
     /etc/init.d/firefly disable
     /etc/init.d/firefly stop
fi
exit 0
endef

$(eval $(call BuildPackage,luci-app-firefly))
